// integrate-batch-A1.js — Replace simplified disease blocks with structured data
const fs = require('fs');
const data = require('./batch-A1-data.js');

const htmlFile = 'vet-differential-diagnosis-v2.html';
let html = fs.readFileSync(htmlFile, 'utf8');

// Helper: convert JS object to unquoted-key format string
...
${pad}]`);
        ...
        // Array of objects (e.g. diagnosticAlgorithm.steps)
        let stepLines = val.map(step => {
          ...
          for (const [sk, sv] of Object.entries(step)) {
            if (typeof sv === 'number') {
              ...
              parts.push(`${sk}: "${sv.replace(/\\/g, '\\\\').replace(/"/g, '\\"')}"`);
      ...
      // Nested object (epidemiology, clinicalPresentation, diagnosis, treatment, diagnosticAlgorithm)
      let innerLines = [];
...
for (const [diseaseId, diseaseData] of Object.entries(data)) {
  // Find the exact old block using brace matching
  ...
  // Find the opening brace of the disease object
  ...
  const afterEnd = html.substring(endIdx, endIdx + 10).trim();
  const hasComma = afterEnd.startsWith(',');
...
try {
  ...
  // Don't rollback automatically, let us investigate
}
